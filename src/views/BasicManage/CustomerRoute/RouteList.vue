...
import ClientRouteAjax from '@/api/RouteManage/ClientRouteMannge'
...
export default {
  ...
  methods: {
    ...
    // },
    queryStations () {
      ClientRouteAjax.QueryStations({logisticsId: logisticsId}).then(response => {
        if (response.code === 200) {
    ...
    queryList (type) {
      ...
      ClientRouteAjax.QueryList(lineQueryParam).then(response => {
        if (response.code === 200) {
    ...
    edit (row) { // 修改信息
      this.$router.push({name: 'CustomerRouteCard', query: { isAdd: false, id: row.id, activeName: 'second' }})
    },
    add () { // 新建线路
      this.$router.push({name: 'CustomerRouteCard', query: { isAdd: true, activeName: 'first' }})
    },
    order (row) {
      this.fileList = []
