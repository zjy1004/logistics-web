        ...
        <div class="searchBar">
          <div class="form-con">
            <el-form :inline="true" ref="form" :model="form" class="demo-form-inline" label-width="95px">
               <el-form-item label="路由归属:" prop="routeAttribution">
                <el-select v-model="form.routeAttribution" @change="logisticsChange('allLogistics')" placeholder="请选择物流公司" clearable>
                  <el-option
              ...
              <el-form-item label="上游起点:" prop="pkStationStart">
                <el-select v-model="form.pkStationStart" placeholder="请选择站点" clearable>
                  <el-option
              ...
              <el-form-item label="" prop="pkStationEnd">
                <el-select v-model="form.pkStationEnd" placeholder="请选择站点" clearable>
                  <el-option
...
export default {
  ...
  data () {
    return {
      ...
      logisticsOption: [], // 下游终点物流公司集合
      destinationStationOption: [], // 下游终点集合
      paginationParams: {pageSize: 10, total: 0, pageNum: 1},
      status: [{id: '', name: '全部'}, {id: 0, name: '启用'}, {id: 1, name: '禁用'}],
      form: {
        routeAttribution: '', // 路线归属物流公司
        pkStationStart: '', // 上游起点
        endRouteAttribution: '', // 下游路线归属物流公司
        pkStationEnd: '', // 下游终点
        enableFlag: '' // 禁用状态
      }
    }
  ...
  methods: {
    ...
    logisticsChange (flag) { // 物流公司变化
      if (flag === 'allLogistics') {
        let logisticsId = this.form.routeAttribution
    ...
    add () { // 新建路由
      this.$router.push({name: 'CreatJointRoute', query: { isAdd: true, activeName: 'first' }})
    },
    enAble (row) { // 禁用
      JoinRouterConfigAjax.EnableRouter({id: row.routeId, enableFlag: row.enableFlag === 0 ? 1 : 0}).then(response => {
        if (response.code === 200) {
          this.$notify({
            message: '保存成功！',
            type: 'success'
