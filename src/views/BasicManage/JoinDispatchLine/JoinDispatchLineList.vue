...
export default {
  ...
  created () {
    ...
    if (userInfo) {
      ...
      this.queryUpStations({logisticsId: this.form.logisticsId})
      this.QueryLogisticsSelectRelation({logisticsIdInitiate: this.form.logisticsId})
    }
    this.init()
  },
  computed: {
    lineType () {
      return this.form.lineType
    }
  },
  watch: {
  ...
  methods: {
    ...
    check (row) { // 查看信息
      this.$router.push({name: 'CheckJoinDispatchLine', query: { id: row.lineId }})
    }
    // edit (row) { // 修改信息
