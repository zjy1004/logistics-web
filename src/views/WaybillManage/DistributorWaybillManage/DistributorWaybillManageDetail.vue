<template>
  <div class="waybillDetail public_card">
    <h3 class="detail_header">运单管理 > 全部运单 ><span class="currentDetail">运单详情</span></h3>
    <div class="card_main">
...
export default {
  ...
  methods: {
    ...
    goBack () {
      this.$router.push({name: 'DistributorWaybillManageList'})
    },
    queryWaybillInfo (val) { // 详情
      WaybillApiAjax.waybillInfo(val).then(res => {
        if (res.code === 200) {
          ...
          if (res.data.waybillClassify === '驮付宝运单' && res.data.paymentMethod === '现金支付') {
            this.waybillInfoData.push({waybillNumber: res.data.waybillNumber, waybillClassify: res.data.waybillClassify, waybillStatus: res.data.waybillStatusName, paymentMethod: ''})
          } else {
