...
export default {
  ...
  props: {
    ...
    isPrintwayBill: { // 是否打印运单
      type: Boolean,
      default: false
    },
    isPrintBoxBill: { // 是否打印箱单
      type: Boolean,
      default: false
    }
  },
  ...
  computed: {
    boxBillCount () {
      let mapArr = []
      for (let i = 0; i < this.printData.packageTotalQuantity; i++) {
        let obj = {
          id: `qrcode${i}`,
          ref: `qrcode${i}`,
          printId: `printBox${i}`
  ...
  methods: {
      ...
      WaybillAjax.NewQueryPrintData({waybillId: this.waybillId}).then((response) => {
        if (response.code === 200) {
          ...
          this.$nextTick(() => {
            ...
            // JsBarcode('#barcode', this.firstPrint.waybillNumber, {width: 1, height: 50, displayValue: false, margin: 0, marginTop: -10, marginLeft: -10})
            // JsBarcode('#barcode1', this.firstPrint.waybillNumber, {width: 1, height: 50, displayValue: false, margin: 0, marginTop: -10, marginLeft: -10})
            // JsBarcode('#barcode2', this.firstPrint.waybillNumber, {width: 1, height: 50, displayValue: false, margin: 0, marginTop: -10, marginLeft: -10})
    ...
    printBill (printData) {
      if (this.isPrintwayBill || this.isPrintBoxBill) {
        this.printLoading() // 打印loading
      }
      setTimeout(() => {
        if (this.isPrintwayBill && this.isPrintBoxBill) { // 运单箱单都打印
          // 打印三联
          this.printThreeBill(printData)
          // 打印箱单
          setTimeout(() => { // 箱单延迟打印
            this.boxBillCount.forEach((item, index) => {
              this.printBoxBillFn(item.printId)
        ...
        } else if (this.isPrintwayBill && !this.isPrintBoxBill) { // 打印运单
          // 打印三联
          this.printThreeBill(printData)
          setTimeout(() => {
            this.$router.go(0)
          }, 1500)
        } else if (!this.isPrintwayBill && this.isPrintBoxBill) { // 打印箱单
          // 打印箱单
          this.boxBillCount.forEach((item, index) => {
            this.printBoxBillFn(item.printId)
            if (index === (this.boxBillCount.length - 1)) { // 最后一个打印完后清除二维码
    ...
    printThreeBill (printData) {
      if (printData.printTypeOne.length > 0) { // 需要打印一联
        this.printThreeWayBillFn('printId1')
      }
      printData.printTypeOther.forEach((item, index) => { // 打印二三联
        if (item.printId === 'printId2') {
          this.printThreeWayBillFn(item.printId) // 打印二联
        }
        if (item.printId === 'printId3') {
          this.printThreeWayBillFn(item.printId) // 打印三联
        }
      })
    ...
    // 打印一联
    // printFirstWayBillFn (elementId) {
    //   let printData = this.printData
    //   let LODOP = getLodop()
    //   LODOP.PRINT_INIT('')
    //   LODOP.ADD_PRINT_RECT(5, 1, 500, 210, 0, 1) // 外边框
    //   LODOP.SET_PRINT_STYLE('Bold', 1)
    //   LODOP.SET_PRINT_STYLE('FontName', '黑体')
    //   LODOP.SET_PRINT_STYLEA(0, 'AngleOfPageInside', -90) // 最新打印机顺时针旋转90度
    //   LODOP.SET_PRINT_STYLE('FontSize', 12)
    //   // 一联左侧
    //   LODOP.SET_PRINT_STYLEA(0, 'Color', '#000000')
    //   LODOP.ADD_PRINT_TEXT(10, 5, 320, 15, `${printData.logisticsName} (${printData.logisticsPhone})`) // 物流公司
    //   LODOP.ADD_PRINT_LINE(30, 0, 30, 320, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(35, 5, 95, 15, `发货方信息`) // 发货方信息
    //   LODOP.SET_PRINT_STYLE('Alignment', 3)
    //   LODOP.ADD_PRINT_TEXT(35, 95, 225, 15, `${printData.articleNumber}（${printData.packageTotalQuantity}件）`)
    //   LODOP.ADD_PRINT_LINE(55, 0, 55, 500, 0, 1)
    //   LODOP.ADD_PRINT_LINE(55, 70, 215, 70, 0, 1)
    //   LODOP.SET_PRINT_STYLE('FontSize', 8)
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.ADD_PRINT_TEXT(60, 0, 70, 15, `发货方`) // 发货方label
    //   LODOP.SET_PRINT_STYLE('Alignment', 1)
    //   LODOP.ADD_PRINT_TEXT(60, 75, 245, 15, `${printData.sendClientName}`) // 发货方值
    //   LODOP.ADD_PRINT_LINE(80, 0, 80, 500, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.ADD_PRINT_TEXT(85, 0, 70, 15, `电话`) // 电话label
    //   LODOP.ADD_PRINT_TEXT(85, 70, 90, 15, `${printData.sendPhone}`) // 电话值
    //   LODOP.ADD_PRINT_LINE(80, 160, 105, 160, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(85, 160, 50, 15, `站点`) // 站点label
    //   LODOP.ADD_PRINT_LINE(80, 210, 105, 210, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(85, 210, 110, 15, `${printData.sendStationName}`) // 站点值
    //   LODOP.ADD_PRINT_LINE(105, 0, 105, 320, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(110, 0, 70, 15, `联系地址`) // 联系地址label
    //   LODOP.ADD_PRINT_TEXT(110, 70, 250, 15, `${printData.sendAddress}`) // 联系地址值
    //   LODOP.ADD_PRINT_LINE(130, 0, 130, 500, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(135, 0, 70, 15, `代收金额`) // 代收金额label
    //   LODOP.ADD_PRINT_TEXT(135, 70, 60, 15, `${printData.receivableGoods}`) // 代收金额值
    //   LODOP.ADD_PRINT_LINE(130, 130, 190, 130, 0, 1)
    //   LODOP.ADD_PRINT_LINE(150, 0, 150, 130, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(155, 0, 70, 15, `运费金额`) // 代收金额label
    //   LODOP.ADD_PRINT_TEXT(155, 70, 60, 15, `${printData.receivableFreight}`) // 运费金额值
    //   LODOP.ADD_PRINT_LINE(170, 0, 170, 130, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(175, 0, 70, 15, `付款方`) // 付款方label
    //   LODOP.ADD_PRINT_TEXT(175, 70, 60, 15, `${printData.freightPayer}`) // 付款方值
    //   LODOP.ADD_PRINT_LINE(190, 0, 190, 320, 0, 1)
    //   // 条形码
    //   LODOP.SET_PRINT_STYLEA(0, 'Color', '#000000')
    //   LODOP.ADD_PRINT_BARCODE(135, 155, 180, 35, 'Code39', `${printData.barcodeNumber}`)
    //   LODOP.SET_PRINT_STYLEA(0, 'ShowBarText', 0)
    //   LODOP.ADD_PRINT_TEXT(175, 143, 150, 15, `${printData.waybillNumber}`) // 运单号
    //   LODOP.ADD_PRINT_TEXT(195, 0, 70, 15, `应收金额`) // 应收金额label
    //   LODOP.ADD_PRINT_TEXT(195, 70, 90, 15, `${printData.waybillReceivable}`) // 应收金额值
    //   LODOP.ADD_PRINT_LINE(190, 160, 215, 160, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(195, 160, 70, 15, `商品数量`) // 商品数量label
    //   LODOP.ADD_PRINT_LINE(190, 230, 215, 230, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(195, 230, 90, 15, `${printData.packageTotalQuantity}`) // 商品数量值
    //   // 一联右侧
    //   LODOP.ADD_PRINT_LINE(5, 320, 215, 320, 0, 1)
    //   LODOP.SET_PRINT_STYLE('FontSize', 8)
    //   LODOP.ADD_PRINT_TEXT(10, 320, 90, 50, `开单时间：${printData.createTime}`) // 开单时间
    //   LODOP.ADD_PRINT_LINE(5, 410, 55, 410, 0, 1)
    //   LODOP.SET_PRINT_STYLE('FontSize', 10)
    //   LODOP.ADD_PRINT_TEXT(10, 410, 90, 50, `班次：${printData.shiftRunName}`) // 班次
    //   LODOP.ADD_PRINT_LINE(55, 320, 55, 500, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 1)
    //   LODOP.ADD_PRINT_TEXT(60, 325, 180, 15, `收货方信息`) // 收货方信息
    //   LODOP.ADD_PRINT_LINE(55, 320, 55, 500, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.ADD_PRINT_TEXT(100, 320, 70, 50, `收货方`) // 收货方label
    //   LODOP.ADD_PRINT_LINE(80, 390, 170, 390, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 1)
    //   LODOP.SET_PRINT_STYLE('FontSize', 10)
    //   LODOP.ADD_PRINT_TEXT(85, 395, 110, 50, `${printData.receiveClientName}`) // 收货方值
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.SET_PRINT_STYLE('FontSize', 8)
    //   LODOP.ADD_PRINT_TEXT(135, 320, 70, 15, `所属站点`) // 所属站点label
    //   LODOP.ADD_PRINT_TEXT(135, 390, 110, 15, `${printData.receiveStationName}`) // 所属站点值
    //   LODOP.ADD_PRINT_LINE(150, 320, 150, 500, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(155, 320, 70, 15, `电话`) // 电话label
    //   LODOP.ADD_PRINT_TEXT(155, 390, 110, 15, `${printData.receivePhone}`) // 电话值
    //   LODOP.ADD_PRINT_LINE(170, 320, 170, 500, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 1)
    //   LODOP.ADD_PRINT_TEXT(172, 325, 180, 50, `备注：${printData.remark}`) // 备注
    //   // 配置打印选项
    //   LODOP.SET_PRINT_MODE('RESELECT_PRINTER', true) // 允许重选打印机
    //   LODOP.SET_PRINTER_INDEX('GP-80160(Cut) Series')
    //   LODOP.SET_LICENSES('', '55C6FFA97D89D4EEFBCA3B741EF2FE49', 'C94CEE276DB2187AE6B65D56B3FC2848', '')
    ...
    //   LODOP.SET_PRINT_STYLE('FontSize', 8)
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.ADD_PRINT_TEXT(10, 0, 90, 50, `开单时间：${printData.createTime}`) // 开单时间
    //   LODOP.ADD_PRINT_LINE(5, 90, 55, 90, 0, 1)
    //   LODOP.SET_PRINT_STYLE('FontSize', 8)
    //   LODOP.ADD_PRINT_TEXT(10, 90, 90, 50, `班次：${printData.shiftRunName}`) // 班次
    //   LODOP.ADD_PRINT_LINE(55, 0, 55, 180, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 1)
    //   LODOP.ADD_PRINT_TEXT(60, 5, 175, 15, `发货方信息`) // 发货方信息
    //   LODOP.ADD_PRINT_LINE(80, 0, 80, 180, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.SET_PRINT_STYLE('FontSize', 10)
    //   LODOP.ADD_PRINT_TEXT(100, 0, 70, 45, `发货方`) // 发货方label
    //   LODOP.ADD_PRINT_LINE(80, 70, 170, 70, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 1)
    //   LODOP.ADD_PRINT_TEXT(85, 75, 105, 45, `${printData.sendClientName}`) // 发货方值
    //   LODOP.ADD_PRINT_LINE(130, 0, 130, 180, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.SET_PRINT_STYLE('FontSize', 8)
    //   LODOP.ADD_PRINT_TEXT(135, 0, 70, 15, `所属站点`) // 所属站点label
    //   LODOP.ADD_PRINT_TEXT(135, 70, 110, 15, `${printData.sendStationName}`) // 所属站点值
    //   LODOP.ADD_PRINT_LINE(150, 0, 150, 180, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(155, 0, 70, 15, `电话`) // 电话label
    //   LODOP.ADD_PRINT_TEXT(155, 70, 110, 15, `${printData.sendPhone}`) // 电话值
    //   LODOP.ADD_PRINT_LINE(170, 0, 170, 180, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 1)
    //   LODOP.ADD_PRINT_TEXT(172, 5, 180, 50, `备注：${printData.remark}`) // 备注
    //   LODOP.ADD_PRINT_LINE(5, 180, 215, 180, 0, 1)
    //   // 二、三联右侧
    //   LODOP.SET_PRINT_STYLE('FontSize', 12)
    //   LODOP.ADD_PRINT_TEXT(10, 185, 315, 15, `${printData.logisticsName} (${printData.logisticsPhone})`) // 物流公司
    //   LODOP.ADD_PRINT_LINE(30, 180, 30, 500, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(35, 185, 95, 15, `收货方信息`) // 收货方信息
    //   LODOP.SET_PRINT_STYLE('Alignment', 3)
    //   LODOP.ADD_PRINT_TEXT(35, 280, 220, 15, `${printData.articleNumber}（${printData.packageTotalQuantity}件）`)
    //   LODOP.ADD_PRINT_LINE(55, 180, 55, 500, 0, 1)
    //   LODOP.ADD_PRINT_LINE(55, 250, 215, 250, 0, 1)
    //   LODOP.SET_PRINT_STYLE('FontSize', 10)
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.ADD_PRINT_TEXT(60, 180, 70, 15, `收货方`) // 收货方label
    //   LODOP.SET_PRINT_STYLE('Alignment', 1)
    //   LODOP.ADD_PRINT_TEXT(60, 255, 245, 15, `${printData.receiveClientName}`) // 收货方值
    //   LODOP.ADD_PRINT_LINE(80, 180, 80, 500, 0, 1)
    //   LODOP.SET_PRINT_STYLE('Alignment', 2)
    //   LODOP.SET_PRINT_STYLE('FontSize', 8)
    //   LODOP.ADD_PRINT_TEXT(85, 180, 70, 15, `电话`) // 电话label
    //   LODOP.ADD_PRINT_TEXT(85, 250, 90, 15, `${printData.receivePhone}`) // 电话值
    //   LODOP.ADD_PRINT_LINE(80, 340, 105, 340, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(85, 340, 50, 15, `站点`) // 站点label
    //   LODOP.ADD_PRINT_LINE(80, 390, 105, 390, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(85, 390, 110, 15, `${printData.receiveStationName}`) // 站点值
    //   LODOP.ADD_PRINT_LINE(105, 180, 105, 500, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(110, 180, 70, 15, `联系地址`) // 联系地址label
    //   LODOP.ADD_PRINT_TEXT(110, 250, 250, 15, `${printData.receiveAddress}`) // 联系地址值
    //   LODOP.ADD_PRINT_LINE(130, 180, 130, 500, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(135, 180, 70, 15, `代收金额`) // 代收金额label
    //   LODOP.ADD_PRINT_TEXT(135, 250, 60, 15, `${printData.receivableGoods}`) // 代收金额值
    //   LODOP.ADD_PRINT_LINE(130, 310, 190, 310, 0, 1)
    //   LODOP.ADD_PRINT_LINE(150, 180, 150, 310, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(155, 180, 70, 15, `运费金额`) // 代收金额label
    //   LODOP.ADD_PRINT_TEXT(155, 250, 60, 15, `${printData.receivableFreight}`) // 运费金额值
    //   LODOP.ADD_PRINT_LINE(170, 180, 170, 310, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(175, 180, 70, 15, `付款方`) // 付款方label
    //   LODOP.ADD_PRINT_TEXT(175, 250, 60, 15, `${printData.freightPayer}`) // 付款方值
    //   LODOP.ADD_PRINT_LINE(190, 180, 190, 500, 0, 1)
    //   // // 条形码
    //   LODOP.ADD_PRINT_BARCODE(135, 330, 180, 35, 'Code39', `${printData.barcodeNumber}`)
    //   LODOP.SET_PRINT_STYLEA(0, 'ShowBarText', 0)
    //   LODOP.ADD_PRINT_TEXT(175, 323, 150, 15, `${printData.waybillNumber}`) // 运单号
    //   LODOP.ADD_PRINT_TEXT(195, 180, 70, 15, `应收金额`) // 应收金额label
    //   LODOP.ADD_PRINT_TEXT(195, 250, 90, 15, `${printData.waybillReceivable}`) // 应收金额值
    //   LODOP.ADD_PRINT_LINE(190, 340, 215, 340, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(195, 340, 70, 15, `商品数量`) // 商品数量label
    //   LODOP.ADD_PRINT_LINE(190, 410, 215, 410, 0, 1)
    //   LODOP.ADD_PRINT_TEXT(195, 410, 90, 15, `${printData.packageTotalQuantity}`) // 商品数量值
    //   // 配置打印选项
    //   LODOP.SET_PRINT_MODE('RESELECT_PRINTER', true) // 允许重选打印机
    //   LODOP.SET_PRINTER_INDEX('GP-80160(Cut) Series')
    //   LODOP.SET_LICENSES('', '55C6FFA97D89D4EEFBCA3B741EF2FE49', 'C94CEE276DB2187AE6B65D56B3FC2848', '')
    ...
    printBoxBillFn (elementId) {
      ...
      if (printData.articleNumber.length <= 9) {
        ...
        LODOP.ADD_PRINT_BARCODE(95, 254, 85, 85, 'QRCode', `${printData.waybillNumber}`) // 二维码
      } else {
        if (printData.receiveClientName.length <= 7) {
          LODOP.SET_PRINT_STYLE('FontSize', 16)
        ...
        LODOP.ADD_PRINT_TEXT(150, 50, 220, 20, `单号：${printData.waybillNumber}`) // 单号
        LODOP.ADD_PRINT_BARCODE(92, 258, 85, 85, 'QRCode', `${printData.waybillNumber}`) // 二维码
      }
      LODOP.SET_PRINT_MODE('RESELECT_PRINTER', true) // 允许重选打印机
