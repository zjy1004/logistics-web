            ...
            <el-form :inline="true" ref="form" :model="form" class="demo-form-inline" label-width="70px">
              <el-row>
                ...
                <el-col :span="2" :offset="0">
                  <el-form-item label=''>
                    <el-input class="inputWidth" v-model="form.driverName" placeholder="司机" clearable></el-input>
                  </el-form-item>
                </el-col>
          ...
          <div class="table-con">
            <el-table
              ...
              <el-table-column
                ...
                label="揽收司机">
                <template slot-scope="scope">
                  <span :class="{'highColor': currentEditRowId === scope.row.id}">{{scope.row.driverName}}</span>
                </template>
              </el-table-column>
              <el-table-column
                min-width="15%"
                ...
                label="任务模式">
                <template slot-scope="scope">
                  <span :class="{'highColor': currentEditRowId === scope.row.id}">{{scope.row.taskModeName}}</span>
                </template>
              </el-table-column>
              <el-table-column
...
export default {
  ...
  methods: {
    ...
    queryList (type) {
      ...
      CreateTasksAjax.QueryWaybillReceiveTask(queryParam).then(response => {
        if (response.code === 200) {
          this.tableData = response.data.list.map((item, index) => {
            item.rowNum = index + 1
            if (item.taskMode === 1) {
              item.taskModeName = '直配'
            }
            if (item.taskMode === 2) {
              item.taskModeName = '回站'
            }
            return item
          })
    ...
    clickSure (val) {
      CreateTasksAjax.CancelReceiveTask({id: this.cancelId}).then((response) => {
        if (response.code === 200) {
